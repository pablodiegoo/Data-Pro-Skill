...
```bash
INIT=$(node "$HOME/.claude/dps-engine/bin/gsd-tools.cjs" init ingest-docs)
...
Agent({
  ...
  prompt: "
    ...
    - dps-engine/references/doc-conflict-engine.md
    </required_reading>
  "
})
```
...
```bash
node "$HOME/.claude/dps-engine/bin/gsd-tools.cjs" commit \
