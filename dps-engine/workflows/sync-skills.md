...
```bash
INSTALL_JS="$(dirname "$0")/../dps-engine/bin/install.js"
# If running from a global install, resolve relative to the GSD package
INSTALL_JS_GLOBAL="$HOME/.claude/dps-engine/bin/install.js"
...
error: source skills root not found: <path>
       ...
       Run: node ~/.claude/dps-engine/bin/install.js --global --<runtime>
```
Then exit.
