...
<required_reading>
@~/.claude/dps-engine/references/domain-probes.md
@~/.claude/dps-engine/references/gate-prompts.md
@~/.claude/dps-engine/references/universal-anti-patterns.md
</required_reading>
...
```bash
GSD_TOOLS="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}/dps-engine/bin/gsd-tools.cjs"; [ -f "$GSD_TOOLS" ] && GSD_SDK="node $GSD_TOOLS" || { echo "ERROR: gsd-tools.cjs not found at $GSD_TOOLS." >&2; exit 1; }
...
if [ -f "$HOME/.claude/dps-engine/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Read `@~/.claude/dps-engine/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
