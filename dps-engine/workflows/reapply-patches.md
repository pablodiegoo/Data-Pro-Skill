...
Run the verifier as a child process (the gsd-tools binary directory is not required — the script ships under `dps-engine/bin/` in the source repo and is installed to `${GSD_HOME}/dps-engine/bin/`; it is also exposed via the SDK at `sdk/dist/cli.js verify-reapply` when present):
...
VERIFY_OUTPUT="$(node "${GSD_HOME}/dps-engine/bin/verify-reapply-patches.cjs" "${VERIFY_ARGS[@]}")"
VERIFY_STATUS=$?
```
