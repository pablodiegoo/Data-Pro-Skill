...
- If the path contains `/dps-engine/workflows/update.md`, strip that suffix and store the remainder as `PREFERRED_CONFIG_DIR`
...
# runtime directories.
if [ -n "$PREFERRED_CONFIG_DIR" ] && { [ -f "$PREFERRED_CONFIG_DIR/dps-engine/VERSION" ] || [ -f "$PREFERRED_CONFIG_DIR/dps-engine/workflows/update.md" ]; }; then
  ...
  if [ -f "$PREFERRED_CONFIG_DIR/dps-engine/VERSION" ] && grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+' "$PREFERRED_CONFIG_DIR/dps-engine/VERSION"; then
    INSTALLED_VERSION="$(cat "$PREFERRED_CONFIG_DIR/dps-engine/VERSION")"
  else
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/dps-engine/VERSION" ] || [ -f "./$dir/dps-engine/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/dps-engine/VERSION"
    LOCAL_MARKER_FILE="./$dir/dps-engine/workflows/update.md"
...
for entry in "${ORDERED_ENV_RUNTIME_DIRS[@]}"; do
  ...
  if [ -f "$dir/dps-engine/VERSION" ] || [ -f "$dir/dps-engine/workflows/update.md" ]; then
    ...
    GLOBAL_VERSION_FILE="$dir/dps-engine/VERSION"
    GLOBAL_MARKER_FILE="$dir/dps-engine/workflows/update.md"
...
if [ -z "$GLOBAL_RUNTIME" ]; then
  for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
    runtime="${entry%%:*}"
    dir="${entry#*:}"
    if [ -f "$HOME/$dir/dps-engine/VERSION" ] || [ -f "$HOME/$dir/dps-engine/workflows/update.md" ]; then
      GLOBAL_RUNTIME="$runtime"
      GLOBAL_VERSION_FILE="$HOME/$dir/dps-engine/VERSION"
      GLOBAL_MARKER_FILE="$HOME/$dir/dps-engine/workflows/update.md"
...
if [ -z "$GSD_DIR" ]; then
  ...
  LATEST_RESULT="$(node "$GSD_DIR/dps-engine/bin/check-latest-version.cjs" --json 2>/dev/null)"
  LATEST_STATUS=$?
...
EXTRACT_JSON=$(node "$GSD_DIR/dps-engine/scripts/changeset/cli.cjs" extract \
...
if [ "$EXTRACT_EXIT" -eq 2 ]; then
  ...
  CHANGELOG_PREVIEW=$(node "$GSD_DIR/dps-engine/scripts/changeset/cli.cjs" extract \
    --from "$INSTALLED_VERSION" \
    --to "$LATEST_VERSION" \
...
- `dps-engine/` will be wiped and replaced
...
```bash
GSD_TOOLS="$RUNTIME_DIR/dps-engine/bin/gsd-tools.cjs"
CUSTOM_JSON=''
