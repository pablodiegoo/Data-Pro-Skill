...
@~/.claude/dps-engine/references/revision-loop.md
@~/.claude/dps-engine/references/gate-prompts.md
@~/.claude/dps-engine/references/agent-contracts.md
@~/.claude/dps-engine/references/gates.md
</required_reading>
...
GSD_TOOLS="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}/dps-engine/bin/gsd-tools.cjs"
if [ -f "$GSD_TOOLS" ]; then
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@~/.claude/dps-engine/references/tdd.md` so gate enforcement rules are available during planning.
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@~/.claude/dps-engine/references/skeleton-template.md`.
...
2. Parse each resolved ADR through `dps-engine/bin/lib/adr-parser.cjs` (`--input`, `--format`) and collect normalized records.
...
**If found:**
1. Read template: `~/.claude/dps-engine/templates/VALIDATION.md`
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @~/.claude/dps-engine/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@~/.claude/dps-engine/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @~/.claude/dps-engine/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "$HOME/.claude/dps-engine/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
