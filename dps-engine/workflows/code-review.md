...
FLAGS_JSON=$(node -e "
  const { parseCodeReviewFlags } = require('./dps-engine/bin/lib/code-review-flags.cjs');
...
const { resolveFallowBinary } = require('./dps-engine/bin/lib/fallow-runner.cjs');
...
if [ "$FIX_FLAG" = "true" ]; then
  ...
  Workflow(workflow="dps-engine/workflows/code-review-fix.md", args="${FIX_ARGS}")
