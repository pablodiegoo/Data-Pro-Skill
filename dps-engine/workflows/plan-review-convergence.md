...
@$HOME/.claude/dps-engine/references/revision-loop.md
@$HOME/.claude/dps-engine/references/gates.md
@$HOME/.claude/dps-engine/references/agent-contracts.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/dps-engine/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "$HOME/.claude/dps-engine/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "$HOME/.claude/dps-engine/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
