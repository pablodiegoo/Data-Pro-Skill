...
@~/.claude/dps-engine/references/context-budget.md
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @~/.claude/dps-engine/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @~/.claude/dps-engine/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/dps-engine/workflows/execute-plan.md` **inline**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       @~/.claude/dps-engine/references/checkpoints.md
       @~/.claude/dps-engine/references/tdd.md
       @~/.claude/dps-engine/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@~/.claude/dps-engine/references/executor-examples.md'}
       </execution_context>
...
5.6. **Post-merge build & test gate:**
   ...
   Read and execute `dps-engine/workflows/execute-phase/steps/post-merge-gate.md`.
...
`dps-engine/workflows/execute-phase/steps/codebase-drift-gate.md` —
...
Read and follow `~/.claude/dps-engine/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
