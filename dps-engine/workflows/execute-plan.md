...
@~/.claude/dps-engine/references/git-integration.md
</required_reading>
...
See `~/.claude/dps-engine/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/dps-engine/references/checkpoints.md for details.
</step>
...
If `NODE_REPAIR` is `true`: invoke `@./.claude/dps-engine/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/dps-engine/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/dps-engine/templates/summary.md`.
