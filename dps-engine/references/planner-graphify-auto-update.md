...
> Documents how `gsd-planner` and `gsd-phase-researcher` surface the opt-in graphify auto-update state (issue #3347). The status surface lives inside `graphifyStatus()` in `dps-engine/bin/lib/graphify.cjs`; no planner-side prompt changes are required.
...
`graphifyStatus()` in `dps-engine/bin/lib/graphify.cjs` reads `.last-build-status.json` and folds the `running` / `failed` states into the existing `stale: true` signal:
