...
**Input:** Must-have artifact: `dps-engine/workflows/context-bridge.md: >50 lines`

**Output:**

> **L1 (Existence):** EXISTS -- `dps-engine/workflows/context-bridge.md` found, 187 lines.
...
> **L3 (Wired):** WIRED -- Referenced by `dps-engine/workflows/execute-phase.md` at step 3. Called by `agents/gsd-executor.md` via @-reference include. Arguments verified: receives planning directory path matching the expected parameter.
> **Status: PASSED**
...
> **Method:** Ran `grep -n "scope" dps-engine/bin/validate-commit.js` to locate scope validation logic.
...
> **Ran:** `echo "feat(: test" | node dps-engine/bin/validate-commit.js` -- exit code 0 (accepted).
...
> **Method:** Ran `grep -n "feedback-loop\|extractFeedback" dps-engine/bin/check-subagent-output.js`.
...
> **Cross-check:** `grep -rn "require.*feedback-loop" dps-engine/bin/` returns zero results across all scripts.
...
> **Method:** Ran `grep "architecture_graph\|architecture_guard" dps-engine/bin/lib/config-schema.json`.
> **Evidence:** Zero matches in schema. However, `grep "architecture" dps-engine/bin/lib/config.cjs` shows both flags in CONFIG_DEFAULTS (lines 426, 428).
...
> **Method:** Ran `grep -rn "\/\[a-zA-Z\]\/" dps-engine/bin/` to find inline MSYS regex.
...
**Input:** Must-have artifact: `dps-engine/bin/lib/audit-checks/index.cjs: >100 lines`

**Output:**

> **L1 (Existence):** EXISTS -- File found at `dps-engine/bin/lib/audit-checks/index.cjs`, 142 lines.
> **Status: PASSED**
