...
@~/.claude/dps-engine/references/planner-source-audit.md for full format, examples, and gap-handling rules.
...
@~/.claude/dps-engine/references/planner-source-audit.md for constraint examples.
...
**Test:** Could a different Claude instance execute without asking clarifying questions? If not, add specificity. See @~/.claude/dps-engine/references/planner-antipatterns.md for vague-vs-specific comparison table.

## TDD Detection

**When `workflow.tdd_mode` is enabled:** Apply TDD heuristics aggressively — all eligible tasks MUST use `type: tdd`. Read @~/.claude/dps-engine/references/tdd.md for gate enforcement rules and the end-of-phase review checkpoint format.
...
**When `MVP_MODE` is enabled (passed by the plan-phase orchestrator):** Decompose tasks as **vertical feature slices**, not horizontal layers. Required reading: `@~/.claude/dps-engine/references/planner-mvp-mode.md` (loaded conditionally by the orchestrator).
...
1. Frame the phase goal as a user story at the top of `PLAN.md`. The user story is sourced from the `**Goal:**` line in ROADMAP.md (set by `mvp-phase`). Emit it with bolded keywords:
   ...
   **As a** [user role], **I want to** [capability], **so that** [outcome].
   ...
   Format rules from `@~/.claude/dps-engine/references/user-story-template.md`:
...
<execution_context>
@~/.claude/dps-engine/workflows/execute-plan.md
@~/.claude/dps-engine/templates/summary.md
</execution_context>
...
See `dps-engine/references/planner-interface-context.md` for the full interface extraction guide.
...
@~/.claude/dps-engine/references/planner-antipatterns.md
...
<gap_closure_mode>
See `dps-engine/references/planner-gap-closure.md`. Load this file at the
start of execution when `--gaps` flag is detected or gap_closure mode is active.
</gap_closure_mode>

<revision_mode>
See `dps-engine/references/planner-revision.md`. Load this file at the
start of execution when `<revision_context>` is provided by the orchestrator.
</revision_mode>

<reviews_mode>
See `dps-engine/references/planner-reviews.md`. Load this file at the
...
- If `--gaps` flag or gap_closure context present: Read `dps-engine/references/planner-gap-closure.md`
- If `<revision_context>` provided by orchestrator: Read `dps-engine/references/planner-revision.md`
- If `--reviews` flag present or reviews mode active: Read `dps-engine/references/planner-reviews.md`
...
```bash
node "$HOME/.claude/dps-engine/bin/gsd-tools.cjs" graphify status
```
...
```bash
node "$HOME/.claude/dps-engine/bin/gsd-tools.cjs" graphify query "<phase-goal-keyword>" --budget 2000
```
...
@~/.claude/dps-engine/references/thinking-models-planning.md
...
See @~/.claude/dps-engine/references/planner-chunked.md for `## OUTLINE COMPLETE` and `## PLAN COMPLETE` return formats used in chunked mode.
