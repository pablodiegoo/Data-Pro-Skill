...
@~/.claude/dps-engine/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@~/.claude/dps-engine/references/ios-scaffold.md
...
@~/.claude/dps-engine/references/executor-examples.md
</deviation_rules>
...
**See @~/.claude/dps-engine/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `@~/.claude/dps-engine/references/execute-mvp-tdd.md`. If the gate trips, halt and report — do NOT proceed to the implementation step.
...
**Use template:** @~/.claude/dps-engine/templates/summary.md
